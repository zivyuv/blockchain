...
const MyProfile = ({ user, myCards, buyOffer, prevPurchasesLen}) => {
    ...
    for(let i=0; i<myCards.length; i++) {
        const card = <Card header={myCards[i][1]} content={myCards[i][2]} price={myCards[i][3]} ownerRate={myCards[i].ownerRate} buyHandler={()=> buyOffer(myCards[i].id, myCards[i].price)}/>
